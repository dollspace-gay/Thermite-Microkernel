...
| D-028 | the first owned raw-map capsule frees its allocation before returning and therefore observes but does not retain an exit key | proves acquisition, decoding, and all-path cleanup without falsely treating a `FreePool`-staled key as suitable for `ExitBootServices`; final retained storage is a separate gate |
...
| G-027 | closed for owned acquisition and raw descriptor decoding | the loader must allocate a bounded map buffer, survive allocation-induced map growth, decode every arbitrary-stride UEFI 2.11 descriptor without unchecked references, connect exact bytes to the Thermite policy, and release ownership on all returns | strict L3 content-preserving `&[u8]` decoder with checked offsets and three replayed receipts; exact 1016-byte direct-Verus probe/allocate/map/scan/free capsule; explicit conditional table/stack/target/buffer assumptions; three model/PE/FAT reproductions; real OVMF TCG+KVM execution; retain final buffer/key ownership, stale-key reacquisition, `ExitBootServices`, and `BootInfoV1` transfer as separate gates |
...
scalar/frame cross-check, transactional policy/action model, CR2-retaining
...
EFIAPI call and raw size/status validation. G-027 closes bounded descriptor-
buffer ownership, the second real map call, exact-byte decoding, policy
integration, map-key observation, and all-path release, but deliberately does
not retain a key or call `ExitBootServices`.
