...
const SOURCE: &str = "verus/platform/boot_page_tables.rs";
const CONSUMER: &str = "tests/m1/boot_page_tables_consumer.rs";
const CRATE_NAME: &str = "tmk_boot_page_tables";
const RLIB: &str = "libtmk_boot_page_tables.rlib";
const VERUS_VERSION: &str = "0.2026.05.24.ecee80a";
const RUNTIME_MARKER: &str =
    "M1_BOOT_PAGE_TABLES_OK observation=1023 pages=13 present=21 aligned=4096";
...
pub fn run() -> Result<(), String> {
    ...
    require_file(&source, "boot page-table Verus source")?;
    require_file(&consumer, "boot page-table runtime consumer")?;

    let source_text = fs::read_to_string(&source)
        .map_err(|error| format!("read {}: {error}", source.display()))?;
    let consumer_text = fs::read_to_string(&consumer)
        .map_err(|error| format!("read {}: {error}", consumer.display()))?;
    audit_source(&source_text, &consumer_text)?;

    let work = root.join("build/m1-page-tables");
    if work.exists() {
    ...
    for (index, directory) in builds.iter().enumerate() {
    ...
    for artifact in artifacts.iter().skip(1) {
        ...
        if actual != model_sha {
            return Err(format!(
                "boot page-table model {} is {actual}, expected {model_sha}",
                artifact.display()
            ));
        }
    }
    ...
    fs::write(work.join("report.txt"), &report)
        .map_err(|error| format!("write M1 boot page-table report: {error}"))?;
    print!("{report}");
    println!("evidence={}", work.display());
    Ok(())
}

fn audit_source(source: &str, consumer: &str) -> Result<(), String> {
    for forbidden in [
        "external_body",
        "assume(",
        "admit(",
        ...
        if source.contains(forbidden) {
            return Err(format!(
                "boot page-table Verus source contains forbidden `{forbidden}`"
            ));
        }
    }
    for required in [
        ...
        "pub open spec fn boot_tables_well_formed",
        "pub fn registered_boot_page_tables",
        "pub fn walk_boot_page_tables",
        "pub fn boot_page_table_observation",
        "ensures result == 1023",
    ] {
        if !source.contains(required) {
            return Err(format!("boot page-table source is missing `{required}`"));
        }
    }
    if source.matches("pub entries: [u64; PAGE_ENTRIES]").count() != 1
        || source.matches("PageTablePage { entries:").count() != 13
    {
        return Err(
            "boot page-table source no longer has exactly thirteen table pages".to_string(),
        );
    }
    for forbidden in ["unsafe ", "asm!", "global_asm!", "external_body", "assume("] {
        if consumer.contains(forbidden) {
            return Err(format!(
                "boot page-table consumer contains forbidden `{forbidden}`"
            ));
        }
    }
    for required in [
        "std::mem::size_of::<BootPageTables>()",
        "std::mem::align_of::<BootPageTables>()",
        "assert_eq!(present, 21)",
        "walk_boot_page_tables",
        "M1_BOOT_PAGE_TABLES_OK observation={observation} pages=13 present={present} aligned=4096",
    ] {
        if !consumer.contains(required) {
            return Err(format!("boot page-table consumer is missing `{required}`"));
        }
    }
    Ok(())
}
...
fn build_model(
    ...
    fs::copy(source, &staged).map_err(|error| format!("stage boot page-table model: {error}"))?;
    let output = run_checked(
        &mut verus_command(tools, directory, &format!("{CRATE_NAME}.rs"), true, false),
        "Verus boot page-table proof and codegen",
    )?;
    ...
    if retain_result {
        fs::write(
            ...
            canonical_json(&output.stdout, "boot page-table Verus result")?,
        )
        .map_err(|error| format!("write boot page-table Verus result: {error}"))?;
    }
    let artifact = directory.join(RLIB);
    require_file(&artifact, "compiled boot page-table model")?;
    Ok(artifact)
}
...
fn audit_artifact(tools: &Tools, artifact: &Path, work: &Path) -> Result<(), String> {
    let members = run_checked(
        ...
        "list boot page-table rlib members",
    )?;
    ...
    {
        return Err(format!(
            "boot page-table rlib member inventory changed:\n{member_text}"
        ));
    }
    ...
    let undefined = run_checked(
        ...
        "audit boot page-table undefined symbols",
    )?;
    ...
    {
        return Err(format!(
            "boot page-table executable undefined-symbol set changed: {names:?}"
        ));
    }
    write_combined_output(
    ...
    let defined = run_checked(
        ...
        "audit boot page-table defined symbols",
    )?;
    require_output_fragments(
        &defined.stdout,
        "boot page-table defined symbols",
        &[
            "registered_boot_page_tables",
            "walk_boot_page_tables",
            "boot_page_table_observation",
        ],
    )?;
    write_combined_output(
    ...
    for (index, artifact) in artifacts.iter().enumerate() {
        ...
        run_checked(
            Command::new(&tools.rustc)
                ...
                .arg("-o")
                .arg(&executable),
            "compile separate boot page-table runtime consumer",
        )?;
        let runtime = run_checked(
            Command::new(&executable).current_dir(root),
            "execute boot page-table runtime consumer",
        )?;
        require_output_fragments(
            &runtime.stdout,
            "boot page-table runtime consumer",
            &[RUNTIME_MARKER],
        )?;
        write_combined_output(
            &work.join(format!("runtime-{}.txt", index + 1)),
            &runtime,
            "boot page-table runtime evidence",
        )?;
        executables.push(executable);
    }
    let expected = sha256sum(&executables[0])?;
    for executable in executables.iter().skip(1) {
        let actual = sha256sum(executable)?;
        if actual != expected {
            return Err(format!(
                "boot page-table consumer {} is {actual}, expected {expected}",
                executable.display()
            ));
        }
    }
...
fn run_negative_proofs(tools: &Tools, source: &str, work: &Path) -> Result<(), String> {
    let cases = [
        (
            ...
            "image_pt[0] = 0x0020_1000 | PRESENT;",
        ),
        (
            "data-execute",
            "image_pt[4] = 0x0020_4000 | PRESENT | WRITABLE | NO_EXECUTE;",
            "image_pt[4] = 0x0020_4000 | PRESENT | WRITABLE;",
        ),
        (
            "unexpected-present-entry",
    ...
    for (name, needle, replacement) in cases {
        if source.matches(needle).count() != 1 {
            return Err(format!(
                "boot page-table negative `{name}` target is not unique"
            ));
        }
        ...
        let output = run_expect_failure(
            ...
            &format!("reject boot page-table {name} mutation"),
        )?;
        ...
        {
            return Err(format!(
                "boot page-table {name} mutation did not fail atomically"
            ));
        }
        write_combined_output(
            &work.join(format!("negative-{name}.txt")),
            &output,
            &format!("boot page-table {name} negative"),
        )?;
    }
    Ok(())
}
...
fn run_no_vstd_boundary(tools: &Tools, source: &Path, work: &Path) -> Result<(), String> {
    ...
    let staged = directory.join(format!("{CRATE_NAME}.rs"));
    fs::copy(source, &staged).map_err(|error| format!("stage no-vstd boundary source: {error}"))?;
    let output = run_expect_failure(
    ...
    require_output_fragments(
        &combined,
        "no-vstd page-table proof boundary",
